...
На этот вопрос можно ответить опираясь на различные точки зрения, причем все они плюс/минус про одном и том же: сокращение paint операций, благодарю выносу в отдельный слой, сокращение задач связанных с рендерингом.
...
Когда-то умельцы из гугла даже целый [csstriggers.com](https://csstriggers.com/) сделали, чтобы понять изменение каких свойств тригерят (создают) задачи в main thread 
...
#### С точки зрения rendering pipeline
...
Если на странице изменится DOM, то мы породим следующую цепочку задач, между 3 процессами участвующими в отрисовке:

<Img src="/assets/posts/2021/better-animations/pipeline-transform.jpg" style="max-width: 600px" />

В случае с анимацией сделанной через `transform: translate`` цепочка сократится задач и участвующих в ней потоков сократится.
